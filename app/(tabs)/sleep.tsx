...
// ─── Full-screen Immersive Sound Player Modal (Cinematic Minimalist) ─────────
const { height: SCR_H, width: SCR_W } = Dimensions.get('screen');
  ...
  useEffect(() => {
    ...
    const generateWave = () => {
      ...
      for (let i = 0; i <= segments; i++) {
        ...
        const maxAmp = (isPlaying && !isPaused) ? 35 : 1;
        // Combine sine waves and some noise for a jagged but organic feel
        const noise = (Math.random() - 0.5) * 0.4;
        const wave1 = Math.sin(i * 0.4 - time * 2);
        const wave2 = Math.cos(i * 0.7 + time * 1.5) * 0.5;
        const yVal = wave1 + wave2 + noise;
        const y = (W / 2) + yVal * maxAmp * Math.pow(envelope, 1.2);
    ...
    generateWave();
    if (isPlaying && !isPaused) {
      interval = setInterval(generateWave, 70); // ~14fps for cinematic jitter
    }
    return () => clearInterval(interval);
  }, [isPlaying, isPaused, CIRCLE_SIZE]);

  const cycleTimer = () => {
    // Cycle through STOP_TIMES
    const nextIdx = (stopIdx + 1) % STOP_TIMES.length;
    onChangeTimer(nextIdx);
  };
      ...
      <Animated.View style={{ flex: 1, transform: [{ translateY: slideAnim }] }}>
        <ImageBackground
          ...
          {!imgUri && (!imgBundledModal) && (
            <LinearGradient colors={['#0F0F0F', '#050505', '#000000']} style={StyleSheet.absoluteFillObject} />
          )}

          <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1 }}>
            {/* Top bar */}
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 24, paddingTop: 16 }}>
              <TouchableOpacity onPress={handleClose} style={{ padding: 8, marginLeft: -8 }}>
                <Ionicons name="chevron-down" size={32} color="rgba(255,255,255,0.8)" />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleClose} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 8 }}>
                <Ionicons name="grid-outline" size={16} color="rgba(255,255,255,0.8)" />
                <Text style={{ fontSize: 13, fontWeight: '400', color: 'rgba(255,255,255,0.8)', letterSpacing: 0.5 }}>Library</Text>
              </TouchableOpacity>
            </View>

            {/* Center Art - Micro-thin circle & delicate waveform */}
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              <View style={{ width: CIRCLE_SIZE, height: CIRCLE_SIZE }}>
                <Svg width="100%" height="100%" viewBox={`0 0 ${CIRCLE_SIZE} ${CIRCLE_SIZE}`}>
                  {/* Micro-thin circle */}
                  <SvgCircle 
                    cx={CIRCLE_SIZE / 2} cy={CIRCLE_SIZE / 2} r={(CIRCLE_SIZE / 2) - 1} 
            ...
            {/* Bottom Section */}
            <View style={{ paddingHorizontal: 32, paddingBottom: 24, alignItems: 'center' }}>
              
              {/* Title & Info */}
              <Text style={{ fontSize: 24, fontWeight: '700', color: '#FFFFFF', letterSpacing: -0.5, marginBottom: 8, textAlign: 'center' }}>
                {sound.label} <Text style={{fontWeight: '300'}}>• {sound.cat}</Text>
              </Text>
              
              {/* Timer Pill */}
              <TouchableOpacity onPress={cycleTimer} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 40 }}>
                <Ionicons name="repeat-outline" size={14} color="rgba(255,255,255,0.6)" />
                <Text style={{ fontSize: 13, fontWeight: '400', color: 'rgba(255,255,255,0.6)', letterSpacing: 0.5 }}>
                  {STOP_TIMES[stopIdx]?.label || 'Timer'}
                </Text>
              </TouchableOpacity>

              {/* Playback Controls */}
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 40, width: '100%', marginBottom: 36 }}>
                <TouchableOpacity onPress={() => { /* prev - decorative */ }} style={{ padding: 10 }}>
                  <Ionicons name="play-skip-back-outline" size={26} color="#FFFFFF" style={{ opacity: 0.8 }} />
                </TouchableOpacity>
                
                <TouchableOpacity onPress={isPlaying ? onToggle : onPlay} style={{ padding: 10 }}>
                  <Ionicons name={(isPlaying && !isPaused) ? "pause-outline" : "play-outline"} size={42} color="#FFFFFF" />
                </TouchableOpacity>
                
                <TouchableOpacity onPress={() => { /* next - decorative */ }} style={{ padding: 10 }}>
                  <Ionicons name="play-skip-forward-outline" size={26} color="#FFFFFF" style={{ opacity: 0.8 }} />
                </TouchableOpacity>
              </View>

              {/* Minimal Progress Bar */}
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, width: '100%' }}>
                <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: 'System', fontVariant: ['tabular-nums'] }}>
                  {isPlaying ? fmtTimer(sessionSecs) : '0:00'}
                </Text>
                
                <View style={{ flex: 1, height: 12, justifyContent: 'center' }}>
                  {/* Track */}
                  <View style={{ width: '100%', height: 1.5, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 1 }} />
                  {/* Progress (fake/placeholder or based on sessionSecs if we know total, but typically endless) */}
                  <View style={{ position: 'absolute', left: 0, width: isPlaying ? '30%' : '0%', height: 1.5, backgroundColor: 'rgba(255,255,255,0.85)', borderRadius: 1 }} />
                  {/* Thumb */}
                  <View style={{ position: 'absolute', left: isPlaying ? '30%' : '0%', width: 6, height: 6, borderRadius: 3, backgroundColor: '#FFFFFF', marginLeft: -3 }} />
                </View>
                
                <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: 'System', fontVariant: ['tabular-nums'] }}>
                  {STOP_TIMES[stopIdx]?.label || '0:00'}
                </Text>
              </View>
...
// Generates an audio-reactive horizontal waveform path inside a 240×240 circle.
// vol  [0..1] drives spike amplitude; phase scrolls the waveform forward each frame.
const buildWaveform = (vol: number, phase: number, numPoints = 120): string => {
  ...
  for (let i = 0; i <= numPoints; i++) {
    ...
    // Taper: amplitude → 0 at the two edge points so the path stays inside the ring
    const taper = Math.pow(Math.cos(nx * Math.PI / 2), 1.5);
    // Three offset sine/cosine harmonics create a rich, chaotic-looking waveform
    ...
    // Alternate sign per-sample produces the rapid zigzag oscilloscope look
    const sign = i % 2 === 0 ? 1 : -1;
    const spike = Math.pow(Math.abs(noise), 1.2) * sign;
    const yOffset = spike * (8 + 90 * vol) * taper;
    const y = CY + yOffset;
    path += (i === 0 ? 'M ' : 'L ') + x.toFixed(2) + ' ' + y.toFixed(2) + ' ';
  }
  return path;
};
// Pre-built constants used by the RAF loop
const WAVE_RESTING = buildWaveform(0.02, 0); // Gentle idle state
const WAVE_HIDDEN  = 'M 120 120 Z';          // Invisible safe fallback
...
// Actually we can just use TouchableWithoutFeedback directly.
  ...
  return (
    <TouchableWithoutFeedback onPress={handleScreenTap}>
      ...
      <LinearGradient
        colors={['rgba(0,0,0,0.7)', 'rgba(0,0,0,0.15)', 'rgba(0,0,0,0.15)', 'rgba(0,0,0,0.85)']}
      ...
      >
        <View style={{
          ...
          <Ionicons
            name={(isPlaying && !isPaused) ? 'play-outline' : 'pause-outline'}
      ...
      >
        ...
        {/* Floating Mini-Player - ALWAYS VISIBLE, LUXURY MINIMALIST */}
        <View style={{ alignItems: 'center', justifyContent: 'center', marginBottom: 32, gap: 20 }}>
          ...
          >
            {(() => {
              ...
              return (
                <>
                  <Ionicons name={opt.icon} size={13} color="rgba(255,255,255,0.9)" />
                  <Text style={{ fontSize: 12, fontWeight: '600', color: 'rgba(255,255,255,0.9)', letterSpacing: 0.5 }}>{opt.label}</Text>
                </>
              );
            })()}
          </TouchableOpacity>

          {/* Transport Controls - Standalone */}
          <View style={{
            ...
            >
              <Ionicons name="play-skip-back-outline" size={28} color="#FFFFFF" />
            </TouchableOpacity>
            ...
            >
              {isActive && isAudioLoading && !isPlaying ? (
                ...
                <Ionicons
                  name={(isPlaying && !isPaused) ? 'pause-outline' : 'play-outline'}
                  size={52} color="#FFFFFF"
                  style={{ marginLeft: (isPlaying && !isPaused) ? 0 : 2 }}
                />
              )}
            </TouchableOpacity>
...
const DashboardHeaderCard = memo(function DashboardHeaderCard() {
  const { solarTimes, accentColor } = useBgContext();
  ...
  useEffect(() => {
    ...
    if (solarTimes) {
      ...
      if (h < sunrise - 1.5 || h >= sunset + 2) setGreeting('Good Night');
      else if (h < sunrise + 2) setGreeting('Good Morning');
      else if (h < solarNoon + 1) setGreeting('Good Morning');
      else if (h < sunset - 1.5) setGreeting('Good Afternoon');
      else setGreeting('Good Evening');
    } else {
      const hr = d.getHours();
      if (hr < 4 || hr >= 21) setGreeting('Good Night');
  ...
  const acc = '#818cf8'; // Premium deep-space indigo accent
...
const RecentlyPlayedPremiumStrip = memo(function RecentlyPlayedPremiumStrip({
  ...
  const sounds = soundIds.length > 0 
    ...
    <Animated.View style={{ width: cardW, transform: [{ scale: scaleAnim }] }}>
      ...
      >
        <View style={{
          ...
          borderWidth: isPlaying ? 1.5 : 1,
          borderColor: isPlaying ? themeColor + 'CC' : themeColor + '30',
          backgroundColor: '#05050F',
          shadowColor: themeColor,
          shadowOffset: { width: 0, height: isPlaying ? 10 : 5 },
          shadowOpacity: isPlaying ? 0.65 : 0.30,
          shadowRadius: isPlaying ? 24 : 14,
          ...
          {/* Deep-space cinematic gradient overlay */}
          <LinearGradient
            colors={['transparent', 'rgba(5,5,18,0.35)', 'rgba(5,5,18,0.80)']}
            locations={[0, 0.55, 1]}
            style={StyleSheet.absoluteFillObject}
            pointerEvents="none"
          />
          {/* Subtle color tint */}
          <View style={[StyleSheet.absoluteFillObject, { backgroundColor: themeColor, opacity: isPlaying ? 0.12 : 0.06 }]} pointerEvents="none" />
            ...
            <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, alignItems: 'center', justifyContent: 'center' }}>
              <View style={{ overflow: 'hidden', borderRadius: 28 }}>
                <BlurView intensity={45} tint="dark" style={{
                  ...
                  alignItems: 'center', justifyContent: 'center',
                  borderWidth: 1, borderColor: themeColor + '60',
                }}>
                  <Ionicons name="play" size={14} color={themeColor} style={{ marginLeft: 2 }} />
                </BlurView>
              </View>
            </View>
          )}
          ...
          {/* NOW PLAYING badge */}
          {isPlaying && (
            <View style={{ position: 'absolute', top: 9, right: 9, overflow: 'hidden', borderRadius: 10, borderWidth: 1, borderColor: themeColor + '50' }}>
              <BlurView intensity={60} tint="dark" style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, gap: 4 }}>
                ...
                <Text style={{ fontSize: 7, color: themeColor, fontFamily: 'Nunito_800ExtraBold', letterSpacing: 0.8 }}>♪</Text>
              </BlurView>
            </View>
          )}
        </View>

        {/* Text Area Below Image — full name always visible */}
        <View style={{ marginTop: 10, paddingHorizontal: 2 }}>
          <Text
            style={{ fontSize: 12.5, color: '#F0ECFF', fontFamily: 'Nunito_700Bold', lineHeight: 17, letterSpacing: 0.25 }}
          >{sound.label}</Text>
          {sound.desc && (
            <Text
              style={{ fontSize: 10.5, color: 'rgba(200,200,255,0.50)', marginTop: 3, letterSpacing: 0.2, fontFamily: 'Nunito_400Regular', lineHeight: 14 }}
              numberOfLines={2}
            >{sound.desc}</Text>
          )}
        </View>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 32, paddingHorizontal: 20 }}>
            ...
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                ...
                <Text style={{ fontSize: 9, fontWeight: '800', color: collection.themeColor, letterSpacing: 2.5, fontFamily: 'Nunito_700Bold', textTransform: 'uppercase' }}>
                  {collection.subtitle || 'Premium Collection'}
                </Text>
              </View>
              <Text style={{ fontSize: 24, color: '#fff', fontFamily: 'DancingScript_600SemiBold', marginBottom: 2, letterSpacing: 0.5, textShadowColor: collection.themeColor + '40', textShadowOffset: { width: 0, height: 2 }, textShadowRadius: 8 }}>
                {collection.title}
              </Text>
              <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', lineHeight: 15, fontFamily: 'Nunito_400Regular' }} numberOfLines={2}>
          ...
          {!shouldRenderHeavy ? (
            ...
            horizontalRows.map((row, rIdx) => {
              // Each row gets a UNIQUE visual identity — title, accent color, shape, icon
              const ROW_STYLES = [
                { title: 'Ethereal Soundscapes',      color: '#818cf8', icon: 'sparkles',       shape: 'diamond' },
                { title: 'Deep Immersion',             color: '#34d399', icon: 'water',          shape: 'pill'    },
                { title: 'Sacred Harmonies',           color: '#f472b6', icon: 'musical-notes',  shape: 'diamond' },
                { title: 'Restorative Frequencies',   color: '#fbbf24', icon: 'radio',           shape: 'pill'    },
                { title: 'Inner Stillness',            color: '#60a5fa', icon: 'moon',            shape: 'diamond' },
                { title: 'Celestial Echoes',           color: '#c084fc', icon: 'planet',          shape: 'pill'    },
                { title: 'Timeless Resonance',         color: '#f87171', icon: 'infinite',        shape: 'diamond' },
                { title: 'Gentle Awakening',           color: '#4ade80', icon: 'sunny',           shape: 'pill'    },
                { title: 'Mindful Journeys',           color: '#a78bfa', icon: 'compass',         shape: 'diamond' },
                { title: 'Infinite Calm',              color: '#38bdf8', icon: 'leaf',            shape: 'pill'    },
              ];
              const rs = ROW_STYLES[rIdx % ROW_STYLES.length];
              const volumeTitle = rs.title;
              const rowColor = rs.color;
              const rowIcon = rs.icon;
              const isShape = rs.shape === 'diamond';

              return (
                <View key={rIdx} style={{ marginBottom: 32 }}>
                  {/* ── Premium Visually-Distinct Row Header ── */}
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14, paddingHorizontal: 20 }}>
                    {/* Accent shape — alternates between diamond and pill */}
                    {isShape ? (
                      <View style={{
                        width: 8, height: 8, borderRadius: 2,
                        backgroundColor: rowColor,
                        transform: [{ rotate: '45deg' }],
                        shadowColor: rowColor, shadowOpacity: 0.8, shadowRadius: 6,
                      }} />
                    ) : (
                      <View style={{
                        paddingHorizontal: 7, paddingVertical: 2,
                        backgroundColor: rowColor + '25',
                        borderRadius: 20, borderWidth: 1, borderColor: rowColor + '55',
                        flexDirection: 'row', alignItems: 'center', gap: 4,
                      }}>
                        <Ionicons name={rowIcon as any} size={8} color={rowColor} />
                      </View>
                    )}
                    <Text style={{
                      fontSize: isShape ? 11 : 10,
                      color: rowColor,
                      letterSpacing: isShape ? 3 : 2.5,
                      fontFamily: isShape ? 'Nunito_800ExtraBold' : 'Nunito_700Bold',
                      textTransform: 'uppercase',
                    }}>
                      {volumeTitle}
                    </Text>
                    {/* Trailing line that fades into the theme color */}
                    <View style={{ flex: 1, height: StyleSheet.hairlineWidth, backgroundColor: rowColor + '30', borderRadius: 1 }} />
                    {/* Small icon on the right end */}
                    <Ionicons name={rowIcon as any} size={11} color={rowColor + '70'} />
                  </View>
    ...
    <View style={[S.screen, { backgroundColor: '#030308' }]}>
      ...
      <View style={{ flex: 1, zIndex: 1 }}>
        ...
        <Animated.ScrollView
          ...
          style={{ flex: 1 }}
          contentContainerStyle={{ flexGrow: 1, paddingBottom: 120, paddingTop: 4 }}
          showsVerticalScrollIndicator={false}
...
{/* ── Content container ── */}
        <View style={{ flex: 1, backgroundColor: 'transparent', paddingTop: 4 }}>
          ...
          <SonicCollections onSelectCollection={setActiveCollectionId} playingId={playingId} />
      ...
      {isSearching && (
        <Animated.View style={{ position: 'absolute', top: insets.top + 80, left: 0, right: 0, bottom: 0, zIndex: 9, backgroundColor: 'rgba(3,3,13,0.95)' }}>
          <FlatList
