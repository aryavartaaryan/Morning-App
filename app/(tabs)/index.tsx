...
import Svg, { Circle as SvgCircle, Ellipse as SvgEllipse, Path as SvgPath, Rect as SvgRect, Defs, LinearGradient as SvgLinearGradient, Stop, G as SvgG, Line as SvgLine } from 'react-native-svg';
...
import StressScanner from '@/components/StressScanner';
...
function FestivalDetailModal({ festival, onClose }: { festival: Festival; onClose: () => void }) {
  ...
  const tabs: { key: 'spiritual' | 'science' | 'cultural'; label: string; icon: string; content: string | undefined }[] = [
    { key: 'spiritual', label: 'Spiritual', icon: '✦', content: festival.philosophicalDesc },
    { key: 'science',   label: 'Science',   icon: '🔬', content: festival.scienceDesc },
    { key: 'cultural',  label: 'Cultural',  icon: '🏮', content: festival.culturalDesc },
  ];
  ...
  return (
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' }}>
        ...
        <View style={{ backgroundColor: '#0A0C1C', borderTopLeftRadius: 32, borderTopRightRadius: 32, maxHeight: '88%', overflow: 'hidden', borderWidth: 1, borderBottomWidth: 0, borderColor: glowColor + '30' }}>
          <BlurView intensity={100} tint="dark" style={StyleSheet.absoluteFillObject} />
          
          {/* Glow header gradient */}
          <LinearGradient
          ...
          <View style={{ alignItems: 'center', paddingTop: 12, paddingBottom: 4 }}>
            <View style={{ width: 40, height: 4, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 99 }} />
          </View>

          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 24, paddingBottom: 60 }}>
            {/* Type label */}
            <Text style={{ fontSize: 9, fontWeight: '900', color: glowColor, letterSpacing: 2, marginBottom: 12, textTransform: 'uppercase' }}>{typeLabel}</Text>
            
            {/* Emoji + Name */}
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 12 }}>
              <View style={{ width: 72, height: 72, borderRadius: 36, backgroundColor: glowColor + '18', borderWidth: 1.5, borderColor: glowColor + '40', alignItems: 'center', justifyContent: 'center', shadowColor: glowColor, shadowOpacity: 0.4, shadowRadius: 16, elevation: 8 }}>
                <Text style={{ fontSize: 36 }}>{festival.emoji}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 22, fontWeight: '900', color: '#FFF', letterSpacing: 0.3, lineHeight: 28 }}>{festival.name.split(' / ')[0]}</Text>
                {festival.name.includes(' / ') && (
                  <Text style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', marginTop: 2, fontStyle: 'italic' }}>{festival.name.split(' / ')[1]}</Text>
                )}
              </View>
            </View>

            {/* Description */}
            <View style={{ backgroundColor: glowColor + '10', borderRadius: 16, padding: 14, marginBottom: 20, borderWidth: 1, borderColor: glowColor + '20' }}>
              <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', lineHeight: 22 }}>{festival.desc}</Text>
            </View>

            {/* Tabs */}
            <View style={{ flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: 4, marginBottom: 20 }}>
              {tabs.map(tab => (
                <TouchableOpacity
                  key={tab.key}
                  onPress={() => { Haptics.selectionAsync(); setActiveTab(tab.key); }}
                  activeOpacity={0.8}
                  style={{ flex: 1, paddingVertical: 10, borderRadius: 16, alignItems: 'center', backgroundColor: activeTab === tab.key ? (glowColor + '28') : 'transparent',
                    borderWidth: activeTab === tab.key ? 1 : 0, borderColor: activeTab === tab.key ? (glowColor + '50') : 'transparent' }}
                >
                  <Text style={{ fontSize: 10, fontWeight: '900', color: activeTab === tab.key ? glowColor : 'rgba(255,255,255,0.45)', letterSpacing: 0.5 }}>{tab.icon} {tab.label.toUpperCase()}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            {tabs.map(tab => activeTab === tab.key && (
              <View key={tab.key} style={{ backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', minHeight: 120 }}>
                {tab.content ? (
                  <Text style={{ fontSize: 15, color: 'rgba(255,255,255,0.9)', lineHeight: 26, fontWeight: '400' }}>{tab.content}</Text>
                ) : (
                  <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.3)', textAlign: 'center', paddingVertical: 24, fontStyle: 'italic' }}>Coming soon...</Text>
                )}
              </View>
            ))}
...
function VedicCalendarModal({ onClose }: { onClose: () => void }) {
  ...
  const [pickerMode, setPickerMode] = React.useState<'month' | 'year' | 'list'>('month');
  const [activeFestDetail, setActiveFestDetail] = React.useState<Festival | null>(null);
  ...
  const NEBULA_PURPLE = '#9B59B6';
  const NEBULA_VIOLET = '#7C3AED';
  const NEBULA_CYAN = '#06B6D4';
  const NEBULA_GOLD = '#F59E0B';
  const NEBULA_PINK = '#EC4899';
  ...
  const MonthGrid = ({ monthDate, festivalsData, selDate, onDateSelect, isYearMode = false }: { monthDate: Date, festivalsData: any[], selDate: Date, onDateSelect: (d: Date) => void, isYearMode?: boolean }) => {
    ...
    return (
      <View style={{ width: SCREEN_W - 32, marginBottom: isYearMode ? 32 : 0 }}>
        {isYearMode && (
           <Text style={{ fontSize: 18, fontWeight: '800', color: NEBULA_PURPLE, marginBottom: 12, marginLeft: 12, letterSpacing: 1 }}>
             {monthDate.toLocaleString('en-US', { month: 'long' }).toUpperCase()}
           </Text>
        )}
        {!isYearMode && (
           <Text style={{ fontSize: 26, fontWeight: '900', color: '#FFFFFF', textAlign: 'center', marginBottom: 24, letterSpacing: -0.5 }}>
             {monthDate.toLocaleString('en-US', { month: 'long' })}
             <Text style={{ color: NEBULA_PURPLE, fontWeight: '300' }}> {monthDate.getFullYear()}</Text>
           </Text>
        )}
        {/* Weekday headers */}
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, paddingHorizontal: 2 }}>
          {['S','M','T','W','T','F','S'].map((d, i) => (
            <View key={i} style={{ width: `${100/7}%`, alignItems: 'center' }}>
              <Text style={{ fontSize: 11, fontWeight: '800', color: i === 0 ? NEBULA_PINK + 'CC' : 'rgba(255,255,255,0.3)', letterSpacing: 0.5 }}>{d}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', rowGap: 4 }}>
          {days.map((date, i) => {
            if (!date) return <View key={i} style={{ width: `${100/7}%`, height: 62 }} />;
            ...
            return (
              <TouchableOpacity
                key={i}
                activeOpacity={0.7}
                onPress={() => { Haptics.selectionAsync(); onDateSelect(date); }}
                style={{ width: `${100/7}%`, height: 62, alignItems: 'center', justifyContent: 'center' }}
              >
                {isSelected && (
                  <LinearGradient
                    colors={[NEBULA_VIOLET, NEBULA_PURPLE]}
                    style={{ position: 'absolute', width: 46, height: 54, borderRadius: 23 }}
                  />
                )}
                {isToday && !isSelected && (
                  <View style={{ position: 'absolute', width: 46, height: 54, borderRadius: 23, borderWidth: 1.5, borderColor: NEBULA_CYAN + '80' }} />
                )}
                {festMatch && !isSelected && (
                  <View style={{ position: 'absolute', top: 4, right: '12%', width: 6, height: 6, borderRadius: 3, backgroundColor: festMatch.festival.type === 'hindu' ? NEBULA_GOLD : NEBULA_CYAN }} />
                )}
                <Text style={{ fontSize: 18, fontWeight: isSelected || isToday ? '900' : '500', color: isSelected ? '#FFF' : (isToday ? NEBULA_CYAN : (isSunday ? NEBULA_PINK + 'CC' : 'rgba(255,255,255,0.85)')) }}>
                  {date.getDate()}
                </Text>
                <Text style={{ fontSize: 7.5, fontWeight: '700', color: isSelected ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.3)', marginTop: 2, letterSpacing: 0.3 }}>
                  {tithiShort}
                </Text>
              </TouchableOpacity>
            );
  ...
  const renderYearPage = ({ item: yearItem }: { item: number }) => {
    ...
    return (
      <View style={{ width: SCREEN_W - 32, alignItems: 'center', paddingTop: 16 }}>
        <Text style={{ fontSize: 36, fontWeight: '900', color: '#FFF', textAlign: 'center', marginBottom: 24, letterSpacing: 6 }}>
          {yearItem}
        </Text>
  ...
  const renderYearMode = () => (
    <View style={{ flex: 1, justifyContent: 'center' }}>
      ...
      />
      <TouchableOpacity 
        style={{ position: 'absolute', left: -10, top: '40%', width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(124,58,237,0.25)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(124,58,237,0.5)' }}
        onPress={() => {
            const idx = yearsData.indexOf(currentYear);
            if (idx > 0) {
      ...
      >
        <Ionicons name="chevron-back" size={22} color={NEBULA_PURPLE} style={{ marginLeft: -2 }} />
      </TouchableOpacity>
      <TouchableOpacity 
        style={{ position: 'absolute', right: -10, top: '40%', width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(124,58,237,0.25)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(124,58,237,0.5)' }}
        onPress={() => {
            const idx = yearsData.indexOf(currentYear);
      ...
      >
        <Ionicons name="chevron-forward" size={22} color={NEBULA_PURPLE} style={{ marginRight: -2 }} />
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <Modal visible animationType="slide" transparent={false} onRequestClose={onClose}>
      ...
      <View style={{ flex: 1, backgroundColor: '#0A0012' }}>
        <LinearGradient
          colors={['#110022', '#0D001A', '#060010', '#0A0018', '#12002A']}
          start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
          style={StyleSheet.absoluteFillObject}
        />
        {/* Purple nebula glow top-right */}
        <View style={{ position: 'absolute', top: -60, right: -60, width: 280, height: 280, borderRadius: 140, backgroundColor: '#7C3AED', opacity: 0.12 }} />
        {/* Cyan glow bottom-left */}
        <View style={{ position: 'absolute', bottom: 80, left: -80, width: 240, height: 240, borderRadius: 120, backgroundColor: '#06B6D4', opacity: 0.07 }} />
        {/* Faint star dust center */}
        <View style={{ position: 'absolute', top: '30%', left: '20%', width: 180, height: 180, borderRadius: 90, backgroundColor: '#9B59B6', opacity: 0.06 }} />

        <View style={{ paddingTop: 58, paddingHorizontal: 16, flex: 1 }}>
          {/* ── HEADER ── */}
          <View style={{ paddingBottom: 18 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <TouchableOpacity onPress={() => onClose()} style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(124,58,237,0.15)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(124,58,237,0.3)' }}>
                <Ionicons name="arrow-back" size={22} color="rgba(255,255,255,0.85)" />
              </TouchableOpacity>
              
              <View style={{ alignItems: 'center', flex: 1 }}>
                <Text style={{ fontSize: 7.5, fontWeight: '900', color: NEBULA_PURPLE, letterSpacing: 3.5, marginBottom: 5 }}>✦  VEDIC CALENDAR  ✦</Text>
                {/* Mode Pill Switcher */}
                <View style={{ flexDirection: 'row', backgroundColor: 'rgba(124,58,237,0.12)', borderRadius: 28, borderWidth: 1, borderColor: 'rgba(124,58,237,0.28)', overflow: 'hidden', padding: 3 }}>
                  {(['month', 'year', 'list'] as const).map((mode) => (
                    <TouchableOpacity 
                      key={mode}
                      onPress={() => { Haptics.selectionAsync(); setPickerMode(mode); }}
                      style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 22, backgroundColor: pickerMode === mode ? NEBULA_VIOLET : 'transparent', alignItems: 'center' }}
                    >
                      <Text style={{ fontSize: 10, fontWeight: '800', color: pickerMode === mode ? '#FFF' : 'rgba(255,255,255,0.45)', letterSpacing: 0.8 }}>
                        {mode === 'list' ? 'FESTIVALS' : mode.toUpperCase()}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
          ...
          {/* ── SELECTED DATE PANEL ── */}
          <View style={{ marginBottom: 16, borderRadius: 22, overflow: 'hidden' }}>
            <LinearGradient
              colors={['rgba(124,58,237,0.22)', 'rgba(76,29,149,0.15)', 'rgba(6,182,212,0.08)']}
              start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
              style={{ padding: 16, borderRadius: 22, borderWidth: 1, borderColor: 'rgba(124,58,237,0.3)' }}
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <View>
                  <Text style={{ color: 'rgba(255,255,255,0.45)', fontSize: 9, fontWeight: '800', letterSpacing: 1.5, marginBottom: 3 }}>SELECTED DATE</Text>
                ...
                {selFest && (
                   <TouchableOpacity onPress={() => setActiveFestDetail(selFest.festival)} style={{ backgroundColor: selFest.festival.type === 'hindu' ? 'rgba(245,158,11,0.15)' : 'rgba(6,182,212,0.15)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: selFest.festival.type === 'hindu' ? 'rgba(245,158,11,0.35)' : 'rgba(6,182,212,0.35)' }}>
                      <Text style={{ color: selFest.festival.type === 'hindu' ? NEBULA_GOLD : NEBULA_CYAN, fontSize: 10, fontWeight: '800' }}>{selFest.festival.emoji}  {selFest.festival.name.split(' / ')[0]}</Text>
                   </TouchableOpacity>
                )}
              </View>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                {[
                  { label: 'TITHI', value: selectedP.tithiName, color: NEBULA_PURPLE },
                  { label: 'NAKSHATRA', value: selN.name, color: NEBULA_CYAN },
                  { label: 'YOGA', value: selY.name, color: selY.auspicious ? '#10B981' : '#F87171' },
                ].map((item, i) => (
                  <View key={i} style={{ alignItems: 'center', flex: 1 }}>
                    {i > 0 && <View style={{ position: 'absolute', left: 0, top: 4, bottom: 4, width: 1, backgroundColor: 'rgba(255,255,255,0.08)' }} />}
                    <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.4)', fontWeight: '800', letterSpacing: 0.8, marginBottom: 4 }}>{item.label}</Text>
                    <Text style={{ fontSize: 12, color: item.color, fontWeight: '700', textAlign: 'center' }} numberOfLines={1}>{item.value}</Text>
                  </View>
                ))}
              </View>
            </LinearGradient>
          ...
          {pickerMode === 'list' ? (
             <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 11, fontWeight: '900', color: NEBULA_PURPLE, textAlign: 'center', marginBottom: 14, letterSpacing: 2.5 }}>✦  COSMIC FESTIVALS & SACRED DAYS  ✦</Text>
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
                  {festivals.map((fest, idx) => (
                    <TouchableOpacity 
                      ...
                      onPress={() => {
                        Haptics.selectionAsync();
                        setSelectedDate(fest.date);
                        setCurrentMonthDate(new Date(fest.date.getFullYear(), fest.date.getMonth(), 1));
                        setPickerMode('month');
                      }}
                      style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 18, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: fest.festival.type === 'hindu' ? 'rgba(245,158,11,0.2)' : 'rgba(6,182,212,0.2)', backgroundColor: fest.festival.type === 'hindu' ? 'rgba(245,158,11,0.05)' : 'rgba(6,182,212,0.05)' }}>
                      <View style={{ width: 48, alignItems: 'center', borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.08)', marginRight: 14, paddingRight: 14 }}>
                         <Text style={{ color: '#fff', fontSize: 18, fontWeight: '900' }}>{fest.date.getDate()}</Text>
                         <Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 9, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 }}>{fest.date.toLocaleString('en-US', { month: 'short' })}</Text>
                      </View>
                      <View style={{ flex: 1 }}>
                         <Text style={{ color: fest.festival.type === 'hindu' ? NEBULA_GOLD : NEBULA_CYAN, fontSize: 14, fontWeight: '800', marginBottom: 3 }}>{fest.festival.emoji} {fest.festival.name.split(' / ')[0]}</Text>
                         <Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, fontWeight: '500' }} numberOfLines={1}>{fest.festival.name}</Text>
                      </View>
                      <Ionicons name="chevron-forward" size={18} color="rgba(124,58,237,0.6)" />
                    </TouchableOpacity>
                  ))}
                </ScrollView>
             </View>
             ...
             <View style={{ flex: 1 }}>
                <View style={{ flex: 1, justifyContent: 'center' }}>
                  <FlatList
                    ...
                    onMomentumScrollEnd={(e) => {
                       ...
                       if (monthsData[newIndex]) {
                          const newMonth = monthsData[newIndex];
                          setCurrentMonthDate(newMonth);
                          const yearFests = getYearlyFestivals(newMonth.getFullYear());
                          const mFests = yearFests.filter(f => f.date.getMonth() === newMonth.getMonth());
                          setSelectedDate(mFests.length > 0 ? mFests[0].date : newMonth);
                       }
                    }}
                  />
                  <TouchableOpacity 
                    style={{ position: 'absolute', left: -10, top: '40%', width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(124,58,237,0.2)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(124,58,237,0.4)' }}
                    onPress={() => {
                        const newIdx = Math.max(0, currentMonthDate.getMonth() - 1);
                        flatListRef.current?.scrollToIndex({ index: newIdx, animated: true });
                        const newMonth = monthsData[newIdx];
                        setCurrentMonthDate(newMonth);
                        const yearFests = getYearlyFestivals(newMonth.getFullYear());
                        const mFests = yearFests.filter(f => f.date.getMonth() === newMonth.getMonth());
                        setSelectedDate(mFests.length > 0 ? mFests[0].date : newMonth);
                  ...
                  <TouchableOpacity 
                    style={{ position: 'absolute', right: -10, top: '40%', width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(124,58,237,0.2)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(124,58,237,0.4)' }}
                    onPress={() => {
                ...
                {/* Legend */}
                <View style={{ flexDirection: 'row', gap: 16, justifyContent: 'center', paddingVertical: 12 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                    <View style={{ width: 7, height: 7, borderRadius: 3.5, backgroundColor: NEBULA_GOLD }} />
                    <Text style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: '700', letterSpacing: 0.5 }}>HINDU FESTIVAL</Text>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                    <View style={{ width: 7, height: 7, borderRadius: 3.5, backgroundColor: NEBULA_CYAN }} />
                    <Text style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: '700', letterSpacing: 0.5 }}>LUNAR EVENT</Text>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                    <View style={{ width: 7, height: 7, borderRadius: 3.5, backgroundColor: NEBULA_PINK, opacity: 0.8 }} />
                    <Text style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: '700', letterSpacing: 0.5 }}>TODAY</Text>
                  </View>
                </View>
             </View>
          )}
  ...
  return (
    <TouchableOpacity
      ...
      <View style={{ alignSelf: 'flex-start', marginTop: 6 }}>
        <MoonSVG tithiNum={moon.tithiNum} size={34} />
      </View>
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', columnGap: 12, rowGap: 10, marginTop: 10, alignItems: 'flex-start', paddingRight: 4 }}>
          {/* Row 1 */}
          <View style={{ flexBasis: '28%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>TITHI</Text>
            <Text style={{ fontSize: 9, color: '#60a5faCC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{p.tithiName}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }}>Day {p.tithiInPaksha}</Text>
          </View>
          <View style={{ flexBasis: '28%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>STAR</Text>
            <Text style={{ fontSize: 9, color: '#fbbf24CC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{nakshatra.name}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }} numberOfLines={1}>{nakshatra.constellation}</Text>
          </View>
          <View style={{ flexBasis: '30%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>ENERGY</Text>
            <Text style={{ fontSize: 9, color: yoga.auspicious ? '#10b981CC' : '#f87171CC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{yoga.en}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }} numberOfLines={1}>{yoga.name}</Text>
          </View>

          {/* Row 2 */}
          <View style={{ flexBasis: '28%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>VAAR</Text>
            <Text style={{ fontSize: 9, color: vaar.color + 'CC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{vaar.vedicName}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }}>{vaar.planet} Day</Text>
          </View>
          <View style={{ flexBasis: '28%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>PAKSHA</Text>
            <Text style={{ fontSize: 9, color: '#60a5faCC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{p.paksha}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }}>{p.paksha === 'Shukla' ? 'Waxing' : 'Waning'}</Text>
          </View>
          <View style={{ flexBasis: '30%' }}>
            <Text style={{ fontSize: 6, color: '#FFFFFF40', fontWeight: '800', letterSpacing: 0.8 }}>MAAS</Text>
            <Text style={{ fontSize: 9, color: '#fbbf24CC', fontWeight: '800', lineHeight: 13 }} numberOfLines={1}>{vMonth.name}</Text>
            <Text style={{ fontSize: 7, color: '#FFFFFF35', lineHeight: 10 }}>{vMonth.en}</Text>
          </View>
        </View>

        <Text style={[CPL.cta, { marginTop: 8 }]}>See today's cosmic energy  ›</Text>
      </View>

      {/* Chevron badge */}
      <View style={[CPL.badge, { backgroundColor: vaar.color + '18', borderColor: vaar.color + '40', alignSelf: 'center' }]}>
...
function HESStoryModal({ cards, initialIndex, onClose }: {
  ...
  // Clamp initialIndex to valid range defensively
  const safeInitial = cards.length > 0 ? Math.max(0, Math.min(initialIndex, cards.length - 1)) : 0;
  const [idx, setIdx] = useState(safeInitial);
  // Guard: if cards is somehow empty, close immediately
  const total = cards.length;
  const safeIdx = total > 0 ? Math.min(idx, total - 1) : 0;
  const card  = total > 0 ? cards[safeIdx] : null;

  const goNext = () => { if (safeIdx < total - 1) setIdx(safeIdx + 1); else onClose(); };
...
// ══════════════════════════════════════════════════════════════════════════════
// Weather Section — ultra-premium edge-to-edge hero with compact hourly strip
// ══════════════════════════════════════════════════════════════════════════════
function WeatherSection({
  ...
  // Determine dynamic gradient based on weather condition
  const isSunny = weather.condition.toLowerCase().includes('clear') || weather.condition.toLowerCase().includes('sun');
  const isRainy = weather.condition.toLowerCase().includes('rain') || weather.condition.toLowerCase().includes('drizzle');
  
  const bgColors: [string, string, string] = isSunny 
    ? ['rgba(251, 146, 60, 0.35)', 'rgba(251, 146, 60, 0.08)', 'rgba(0,0,0,0.4)'] 
    : isRainy 
      ? ['rgba(59, 130, 246, 0.35)', 'rgba(30, 58, 138, 0.1)', 'rgba(0,0,0,0.4)']
      : ['rgba(255, 255, 255, 0.25)', 'rgba(255, 255, 255, 0.08)', 'rgba(0,0,0,0.4)'];

  const accentColor = isSunny ? '#fb923c' : isRainy ? '#60a5fa' : '#FFFFFF';

  return (
    <View style={[WSEC.container, { shadowColor: accentColor }]}>
      ...
      <View style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 1, backgroundColor: 'rgba(255,255,255,0.1)' }} />
      ...
      <View style={{ padding: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
            <View style={{ 
              ...
              <Text style={{ fontSize: 32 }}>{weather.emoji}</Text>
            </View>
            <View>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                <Text style={{ fontSize: 48, fontWeight: '200', color: '#FFFFFF', letterSpacing: -2, lineHeight: 54 }}>{weather.temp}</Text>
                <Text style={{ fontSize: 24, fontWeight: '300', color: 'rgba(255,255,255,0.5)', marginTop: 4 }}>°</Text>
              </View>
              <Text style={{ fontSize: 13, color: accentColor, fontWeight: '700', letterSpacing: 1.5 }}>{weather.condition.toUpperCase()}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={{ alignItems: 'flex-end', justifyContent: 'center', gap: 8 }}>
          {maxT !== null && (
            <View style={WSEC.metricPill}>
              <Text style={{ fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.5)' }}>H: <Text style={{ color: '#f87171' }}>{maxT}°</Text></Text>
              <View style={{ width: 1, height: 10, backgroundColor: 'rgba(255,255,255,0.2)', marginHorizontal: 6 }} />
              <Text style={{ fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.5)' }}>L: <Text style={{ color: '#93c5fd' }}>{minT}°</Text></Text>
            </View>
          )}
          <View style={WSEC.metricPill}>
            <Text style={{ fontSize: 10, fontWeight: '800', color: '#FFF' }}>💧 {weather.humidity}%</Text>
          </View>
          {hasRain && (
            <View style={[WSEC.metricPill, { backgroundColor: 'rgba(147,197,253,0.15)', borderColor: 'rgba(147,197,253,0.3)' }]}>
              <Text style={{ fontSize: 10, fontWeight: '800', color: '#93c5fd' }}>🌧 {rainMm}mm</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={{ paddingBottom: 20 }}>
        ...
        >
          {weather.hourly?.map((pt, i) => {
            ...
            return (
              <View key={i} style={[WSEC.hourCard, isNow && WSEC.hourCardNow]}>
                ...
                <Text style={[WSEC.hourTime, isNow && { color: '#FFF' }]}>{isNow ? 'NOW' : hrLabel(pt.hour)}</Text>
                <Text style={WSEC.hourEmoji}>{nightAwareEmoji(pt.emoji, pt.hour, solarTimes)}</Text>
                <Text style={[WSEC.hourTemp, isNow && { color: '#FFF' }]}>{pt.temp}°</Text>
                {showRain && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2, marginTop: 6, backgroundColor: 'rgba(147,197,253,0.15)', paddingHorizontal: 5, paddingVertical: 3, borderRadius: 6 }}>
                    <Text style={{ fontSize: 8 }}>💧</Text>
                    <Text style={WSEC.rainPct}>{rp}%</Text>
                  </View>
                )}
              </View>
            );
          ...
          <TouchableOpacity onPress={onMore} activeOpacity={0.8} style={WSEC.moreCard}>
            <LinearGradient
              colors={['rgba(255,255,255,0.12)', 'rgba(255,255,255,0.02)']}
              style={StyleSheet.absoluteFillObject}
            />
            <Text style={{ fontSize: 20 }}>📅</Text>
            <Text style={{ fontSize: 9, fontWeight: '800', color: 'rgba(255,255,255,0.9)', marginTop: 8, letterSpacing: 0.5 }}>14 DAYS</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
    </View>
...
const WSEC = StyleSheet.create({
  ...
  hourTime: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.5)', letterSpacing: 0.5 },
  hourEmoji: { fontSize: 26, marginVertical: 8 },
  hourTemp: { fontSize: 16, fontWeight: '800', color: 'rgba(255,255,255,0.9)' },
  rainPct: { fontSize: 9, fontWeight: '800', color: '#93c5fd' },
  moreCard: { 
    alignItems: 'center', 
...
// ══════════════════════════════════════════════════════════════════════════════
// 14-Day Premium Weather Modal
// ══════════════════════════════════════════════════════════════════════════════
function FourteenDayWeatherModal({ weather, onClose }: { weather: WeatherData, onClose: () => void }) {
  ...
  const handleClose = () => {
    Animated.parallel([
      Animated.spring(slideAnim, { toValue: SCREEN_H, tension: 60, friction: 12, useNativeDriver: true }),
  ...
  const absMax = Math.max(...daily.map(d => d.maxTemp));
  const absMin = Math.min(...daily.map(d => d.minTemp));
  const tempRange = absMax - absMin || 1;

  return (
    <Modal visible transparent animationType="none" onRequestClose={handleClose}>
      <Animated.View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.65)', opacity: fadeAnim }}>
        <TouchableOpacity style={StyleSheet.absoluteFillObject} onPress={handleClose} activeOpacity={1} />
      </Animated.View>

      <Animated.View style={{
        position: 'absolute', bottom: 0, left: 0, right: 0, height: SCREEN_H * 0.85,
        borderTopLeftRadius: 36, borderTopRightRadius: 36, overflow: 'hidden',
        transform: [{ translateY: slideAnim }]
      }}>
        <BlurView intensity={90} tint="dark" style={StyleSheet.absoluteFillObject} />
        <LinearGradient colors={['rgba(20,40,90,0.65)', 'rgba(5,15,35,0.95)', '#050a1a']} start={{x:0, y:0}} end={{x:0, y:1}} style={StyleSheet.absoluteFillObject} />
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1.5, backgroundColor: 'rgba(255,255,255,0.2)' }} />

        <View style={{ alignItems: 'center', paddingTop: 14, paddingBottom: 6 }}>
          <View style={{ width: 48, height: 5, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.25)' }} />
        </View>

        <View style={{ paddingHorizontal: 24, paddingTop: 10, paddingBottom: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <Text style={{ fontSize: 26, fontWeight: '800', color: '#FFF', letterSpacing: -0.5 }}>14-Day Forecast</Text>
          <TouchableOpacity onPress={handleClose} style={{ width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(255,255,255,0.1)', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: '#FFF', fontSize: 16 }}>✕</Text>
          </TouchableOpacity>
        </View>

        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 60 }}>
          <View style={{ backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 24, padding: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' }}>
            {daily.map((day, i) => {
              const d = new Date(day.date);
              const isToday = i === 0;
              const dayStr = isToday ? 'Today' : d.toLocaleDateString('en-US', { weekday: 'short' });
              
              // Bar calculations
              const leftPct = ((day.minTemp - absMin) / tempRange) * 100;
              const widthPct = ((day.maxTemp - day.minTemp) / tempRange) * 100;

              return (
                <View key={i} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 12, borderBottomWidth: i === daily.length - 1 ? 0 : 0.5, borderBottomColor: 'rgba(255,255,255,0.06)' }}>
                  <Text style={{ width: 55, fontSize: 16, fontWeight: isToday ? '800' : '500', color: isToday ? '#60a5fa' : '#FFF' }}>{dayStr}</Text>
                  
                  <View style={{ flexDirection: 'row', alignItems: 'center', width: 45 }}>
                    <Text style={{ fontSize: 22 }}>{day.emoji}</Text>
                    {day.precipitation > 0 && (
                       <Text style={{ fontSize: 10, fontWeight: '700', color: '#93c5fd', marginLeft: 4 }}>{day.precipitation}mm</Text>
                    )}
                  </View>

                  <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', marginLeft: 16 }}>
                    <Text style={{ width: 28, fontSize: 15, fontWeight: '600', color: 'rgba(255,255,255,0.6)', textAlign: 'right', marginRight: 8 }}>{day.minTemp}°</Text>
                    
                    {/* Temperature Bar */}
                    <View style={{ flex: 1, height: 6, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
                      <LinearGradient 
                        colors={['#60a5fa', '#fbbf24']} 
                        start={{x:0, y:0}} end={{x:1, y:0}}
                        style={{ position: 'absolute', top: 0, bottom: 0, left: `${leftPct}%`, width: `${Math.max(8, widthPct)}%`, borderRadius: 3 }} 
                      />
                    </View>

                    <Text style={{ width: 28, fontSize: 15, fontWeight: '700', color: '#FFF', textAlign: 'left', marginLeft: 8 }}>{day.maxTemp}°</Text>
                  </View>
                </View>
              );
            })}
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 52 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 14, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: '#FFFFFF0A', marginBottom: 20 }}>
              <View style={{ flex: 1, gap: 3 }}>
                <Text style={{ fontSize: 8, fontWeight: '900', color: period.color, letterSpacing: 2.2 }}>CURRENT BIOLOGICAL RHYTHM</Text>
...
const ZEN_SOUNDS: PlayableSoundMeta[] = [
  ...
  { id: 'stotra_bhagya',    label: 'Hymn of Fortune (Bhagya Suktam)',   emoji: '🌟', color: '#fde68a', top: '#1A1400', bot: '#0A0A00', cat: 'Stotra', desc: 'Vedic hymn for prosperity',         src: { uri: 'https://audio.onesutralabs.com/sounds-large/bhagya-suktam.m4a' } },
...
// Sunrise palette: deep navy pre-dawn → crimson-rose horizon → gold zenith
const SUNRISE_PALETTES: Array<AyurvedicPalette> = [
  { ring: '#1E3A8A', halo: '#2563EB', accent: '#93C5FD' }, // pre-dawn deep blue
...
function HeroRingDisplay({ period, brahmaInfo, weather, onPress, compact, solarTimes }: { period: DoshaPeriod | null; brahmaInfo?: BrahmaMuhurtaInfo | null; weather?: WeatherData | null; onPress?: () => void; compact?: boolean; solarTimes?: SolarTimes | null }) {
  ...
  const latestProps = useRef({ onPress, compact });
  latestProps.current = { onPress, compact };
  ...
  // Premium animated color transition based on Circadian Period
  const baseColor = period?.color || appAccent || '#00D4B8';
  const [currentColor, setCurrentColor] = React.useState(baseColor);
  const [prevColor, setPrevColor] = React.useState(baseColor);
  ...
  React.useEffect(() => {
    if (baseColor !== currentColor) {
      ...
      Animated.timing(colorAnim, {
        toValue: 1,
        duration: 3500, // Luxurious 3.5s crossfade
        easing: Easing.inOut(Easing.cubic),
        useNativeDriver: false // JS interpolation is safe here since we split nodes
      }).start();
    }
  }, [baseColor, currentColor, colorAnim]);
  
  const accentHex = colorAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [prevColor, currentColor]
  });

  const accentHex60 = colorAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [prevColor + '99', currentColor + '99']
  });
  ...
  return (
    <View style={{ alignItems: 'center', marginTop: compact ? 4 : 6 }}>
      ...
      <View style={{ width: MOON_RS, height: MOON_RS, alignItems: 'center', justifyContent: 'center' }}>
          ...
          {sacredHour.type !== null ? (
            <>
              ...
              <Animated.View style={{
                ...
                <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: '#FFF', shadowColor: '#FFF', shadowOpacity: 1, shadowRadius: 4 }} />
            ...
            <>
              ...
              <Animated.View style={{
                ...
                >{heroContent.header}</Animated.Text>
          ...
          {/* ── Exact hit area overlay for Stories (strictly clipped to inner ring circle) ── */}
  ...
  return (
    <>
      <View style={{
        ...
        <View style={{ position: 'absolute', top: -100, right: -100, width: 400, height: 400, borderRadius: 200, backgroundColor: GOLD, opacity: 0.05 }} />
        <View style={{ position: 'absolute', bottom: -100, left: -100, width: 300, height: 300, borderRadius: 150, backgroundColor: '#3b82f6', opacity: 0.03 }} />
        
        {/* Top Header */}
        <View style={{ alignItems: 'center', marginBottom: 28 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
            <Text style={{ fontFamily: 'Georgia', fontSize: 17, fontWeight: '700', color: GOLD, letterSpacing: 4, textTransform: 'uppercase' }}>
              Jyotish Veda
            </Text>
            {/* Real-time Moon Phase in top right corner */}
            <View style={{ position: 'absolute', right: 0, shadowColor: '#FFF', shadowOpacity: 0.4, shadowRadius: 10, elevation: 4 }}>
              <MoonSVG tithiNum={moon.tithiNum} size={30} />
            </View>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 14, opacity: 0.8 }}>
            <Text style={{ fontSize: 10.5, color: '#FFF', fontWeight: '500', letterSpacing: 0.5 }}>
        ...
        {/* Tithi Section - Premium Glassmorphism */}
        <TouchableOpacity activeOpacity={0.8} onPress={() => toggleSection('tithi')} style={{ marginBottom: 20, borderRadius: 20, overflow: 'hidden', borderWidth: 0.5, borderColor: 'rgba(212,175,55,0.2)' }}>
          <BlurView intensity={20} tint="dark" style={{ padding: 20, alignItems: 'center' }}>
            <Text style={{ fontSize: 9.5, color: 'rgba(212,175,55,0.8)', fontWeight: '800', letterSpacing: 4, marginBottom: 12 }}>
              TODAY'S VEDIC INSIGHT
            </Text>
            
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: '100%', marginBottom: 6 }}>
              <Text style={{ fontFamily: 'Georgia', fontSize: 28, color: '#FFF', fontWeight: '400', letterSpacing: 3 }}>
                TITHI
              </Text>
              <View style={{ position: 'absolute', right: 0 }}>
                <Ionicons name={expandedSection === 'tithi' ? "chevron-up" : "chevron-down"} size={22} color="rgba(212,175,55,0.7)" />
              </View>
            </View>
            
            <Text style={{ fontFamily: 'Georgia', fontSize: 22, color: GOLD, fontWeight: '400', letterSpacing: 0.5, marginBottom: 10, textAlign: 'center' }}>
              {tithiLabel}
            </Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', opacity: 0.9 }}>
              <Text style={{ fontSize: 11.5, color: GOLD_LIGHT, fontWeight: '500', letterSpacing: 0.5 }}>Ends {tithiEnds}</Text>
              <View style={{ width: 1, height: 12, backgroundColor: 'rgba(243,229,171,0.3)', marginHorizontal: 12 }} />
              <Text style={{ fontSize: 11.5, color: GOLD_LIGHT, fontWeight: '500', letterSpacing: 0.5 }}>{moon.illumination}% Illuminated</Text>
            </View>
            ...
            {expandedSection === 'tithi' && (
              <View style={{ marginTop: 24, width: '100%' }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8, paddingHorizontal: 4 }}>
                  <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', fontWeight: '600', letterSpacing: 1 }}>START</Text>
                  <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', fontWeight: '600', letterSpacing: 1 }}>END</Text>
                </View>
                <View style={{ height: 2, backgroundColor: 'rgba(212,175,55,0.2)', borderRadius: 1, width: '100%', marginBottom: 8, flexDirection: 'row', alignItems: 'center' }}>
                  <View style={{ width: '60%', height: 2, backgroundColor: GOLD }} />
                  <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: '#FFF', shadowColor: GOLD, shadowOpacity: 1, shadowRadius: 6, elevation: 4, marginLeft: -4 }} />
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 2 }}>
                  <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500' }}>{tFmt(timings.tithiStart)}</Text>
                  <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500' }}>{tFmt(timings.tithiEnd)}</Text>
                </View>
              </View>
            )}
          </BlurView>
        ...
        <View style={{ alignItems: 'center', justifyContent: 'center', height: 190, marginBottom: 20 }}>
          <Svg width={320} height={190} style={{ position: 'absolute' }}>
            {/* Perfectly calculated SVG Ellipses */}
            <SvgEllipse cx="160" cy="95" rx="45" ry="16" fill="none" stroke="rgba(212,175,55,0.4)" strokeWidth="0.5" />
            <SvgEllipse cx="160" cy="95" rx="80" ry="28" fill="none" stroke="rgba(212,175,55,0.3)" strokeWidth="0.5" />
            <SvgEllipse cx="160" cy="95" rx="115" ry="40" fill="none" stroke="rgba(212,175,55,0.25)" strokeWidth="0.75" />
            <SvgEllipse cx="160" cy="95" rx="150" ry="52.5" fill="none" stroke="rgba(212,175,55,0.15)" strokeWidth="0.5" />
            
            {/* Exactly positioned planets on the orbits */}
            <SvgCircle cx="160" cy="111" r="3.5" fill="#D4AF37" />     {/* r=45, angle=90 */}
            <SvgCircle cx="240" cy="95" r="3" fill="#B0C4DE" />    {/* r=80, angle=0 */}
            <SvgCircle cx="80" cy="95" r="4.5" fill="#F59E0B" />       {/* r=80, angle=180 */}
            <SvgCircle cx="241.3" cy="123.3" r="4" fill="#EC4899" /> {/* r=115, angle=45. x=160+115*.707=241.3, y=95+40*.707=123.3 */}
            <SvgCircle cx="78.7" cy="66.7" r="2.5" fill="#FFFFFF" opacity={0.8} /> {/* r=115, angle=225. x=160-81.3=78.7, y=95-28.3=66.7 */}
            <SvgCircle cx="266.1" cy="57.9" r="3.5" fill="#AA8C2C" />    {/* r=150, angle=315. x=160+150*.707=266.1, y=95-52.5*.707=57.9 */}
          </Svg>
          
          {/* Glowing central SUN perfectly centered at 160, 95 */}
          <View style={{ 
            position: 'absolute', top: 65, left: 130,
            width: 60, height: 60, borderRadius: 30, backgroundColor: '#FDE047', 
            alignItems: 'center', justifyContent: 'center',
            shadowColor: '#FEF08A', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 1, shadowRadius: 30, elevation: 20 
          }}>
            <LinearGradient
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 20 }}>
          <View style={{ flex: 1, padding: 12, backgroundColor: 'rgba(239,68,68,0.05)', borderRadius: 12, borderWidth: 0.5, borderColor: 'rgba(239,68,68,0.2)' }}>
            <Text style={{ fontSize: 9, color: '#FCA5A5', fontWeight: '700', letterSpacing: 1.5, marginBottom: 4 }}>RAHU KAAL</Text>
            <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500' }}>Inauspicious</Text>
            <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginTop: 4 }}>4:30 PM - 6:00 PM</Text>
          </View>
          <View style={{ flex: 1, padding: 12, backgroundColor: 'rgba(52,211,153,0.05)', borderRadius: 12, borderWidth: 0.5, borderColor: 'rgba(52,211,153,0.2)' }}>
            <Text style={{ fontSize: 9, color: '#6EE7B7', fontWeight: '700', letterSpacing: 1.5, marginBottom: 4 }}>ABHIJIT</Text>
            <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500' }}>Highly Auspicious</Text>
            <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginTop: 4 }}>11:45 AM - 12:35 PM</Text>
          </View>
        </View>

        {/* Planetary Alignments Grid */}
        <View style={{ marginBottom: 20, borderRadius: 20, overflow: 'hidden', borderWidth: 0.5, borderColor: 'rgba(212,175,55,0.2)' }}>
          <BlurView intensity={20} tint="dark" style={{ padding: 16 }}>
            <Text style={{ fontSize: 9, color: 'rgba(212,175,55,0.8)', fontWeight: '700', letterSpacing: 3, textAlign: 'center', marginBottom: 20 }}>
              PLANETARY ALIGNMENTS
            </Text>
            
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }}>
              <View style={{ gap: 14 }}>
                <Text style={{ fontSize: 11.5, color: '#FFF', fontWeight: '500', letterSpacing: 0.5 }}><Text style={{ color: GOLD }}>SUN</Text> in {sunRashi} ☀️</Text>
                <Text style={{ fontSize: 11.5, color: '#FFF', fontWeight: '500', letterSpacing: 0.5 }}><Text style={{ color: GOLD }}>MOON</Text> in {nakshatra.name} 🌙</Text>
              </View>
              <TouchableOpacity activeOpacity={0.7} onPress={() => toggleSection('vaar')} style={{ gap: 14, alignItems: 'flex-end' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <Text style={{ fontSize: 11.5, color: '#FFF', fontWeight: '500', letterSpacing: 0.5 }}><Text style={{ color: GOLD }}>VAAR</Text> is {vaar.planet} 🪐</Text>
                  <Ionicons name={expandedSection === 'vaar' ? "chevron-up" : "chevron-down"} size={14} color="rgba(212,175,55,0.8)" />
                </View>
                <Text style={{ fontSize: 11.5, color: '#FFF', fontWeight: '500', letterSpacing: 0.5 }}><Text style={{ color: GOLD }}>MAAS</Text> is {vMonth.name} ✨</Text>
              </TouchableOpacity>
            </View>

            {expandedSection === 'vaar' && (
              <View style={{ marginBottom: 20, padding: 14, backgroundColor: 'rgba(212,175,55,0.06)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(212,175,55,0.15)' }}>
                <Text style={{ fontSize: 11, color: '#FFF', marginBottom: 6, fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>DAY:</Text> {ENGLISH_DAYS[p.vaarIdx]}</Text>
                <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>RULED BY:</Text> {vaar.planet}</Text>
              </View>
            )}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <TouchableOpacity activeOpacity={0.7} onPress={() => toggleSection('nakshatra')} style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <Text style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.6)', fontWeight: '600', letterSpacing: 1.5 }}>NAKSHATRA</Text>
                  <Ionicons name={expandedSection === 'nakshatra' ? "chevron-up" : "chevron-down"} size={12} color="rgba(212,175,55,0.8)" />
                </View>
                <Text style={{ fontFamily: 'Georgia', fontSize: 16, color: '#FFF' }}>{nakshatra.name} <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', fontFamily: 'System' }}>({timeOnly(timings.nakshatraEnd)})</Text></Text>
              </TouchableOpacity>
              
              <TouchableOpacity activeOpacity={0.7} onPress={() => toggleSection('yoga')} style={{ flex: 1, alignItems: 'flex-end' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <Ionicons name={expandedSection === 'yoga' ? "chevron-up" : "chevron-down"} size={12} color="rgba(212,175,55,0.8)" />
                  <Text style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.6)', fontWeight: '600', letterSpacing: 1.5 }}>YOGA</Text>
                </View>
                <Text style={{ fontFamily: 'Georgia', fontSize: 16, color: '#FFF' }}>{yoga.name} <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', fontFamily: 'System' }}>({timeOnly(timings.yogaEnd)})</Text></Text>
              </TouchableOpacity>
            </View>

            {expandedSection === 'nakshatra' && (
              <View style={{ marginTop: 20, padding: 14, backgroundColor: 'rgba(212,175,55,0.06)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(212,175,55,0.15)' }}>
                <Text style={{ fontSize: 11, color: '#FFF', marginBottom: 6, fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>START:</Text> {tFmt(timings.nakshatraStart)}</Text>
                <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>END:</Text> {tFmt(timings.nakshatraEnd)}</Text>
              </View>
            )}

            {expandedSection === 'yoga' && (
              <View style={{ marginTop: 20, padding: 14, backgroundColor: 'rgba(212,175,55,0.06)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(212,175,55,0.15)' }}>
                <Text style={{ fontSize: 11, color: '#FFF', marginBottom: 6, fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>START:</Text> {tFmt(timings.yogaStart)}</Text>
                <Text style={{ fontSize: 11, color: '#FFF', fontWeight: '500', letterSpacing: 1 }}><Text style={{ color: GOLD_LIGHT }}>END:</Text> {tFmt(timings.yogaEnd)}</Text>
              </View>
            )}
          </BlurView>
        </View>

        <View style={{ height: 1, backgroundColor: 'rgba(212,175,55,0.15)', marginBottom: 20 }} />

        {/* Footer: Dynamic Horizon Tracker */}
        <View style={{ alignItems: 'center', marginBottom: 24 }}>
          {/* Horizon Arc */}
          <Svg width={200} height={40} style={{ marginBottom: -10 }}>
            <SvgPath d="M0,35 Q100,5 200,35" fill="none" stroke="rgba(212,175,55,0.2)" strokeWidth="1.5" strokeDasharray="4 4" />
            <SvgCircle cx="130" cy="18" r="4" fill="#FDE047" shadowColor="#FDE047" shadowOpacity={1} shadowRadius={5} />
          </Svg>
          
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            <View>
              <Text style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.5)', fontWeight: '600', marginBottom: 4 }}>SUNRISE</Text>
              <Text style={{ fontSize: 12, color: '#FFF', fontWeight: '600' }}>{fmtSolar(csr)}</Text>
            </View>
            <View style={{ alignItems: 'center' }}>
              <Text style={{ fontSize: 9, color: GOLD, fontWeight: '800', letterSpacing: 1.5, marginBottom: 4 }}>MOON PHASE</Text>
              <Text style={{ fontSize: 12, color: GOLD_LIGHT, fontWeight: '600' }}>{p.paksha} Paksha</Text>
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.5)', fontWeight: '600', marginBottom: 4 }}>SUNSET</Text>
              <Text style={{ fontSize: 12, color: '#FFF', fontWeight: '600' }}>{fmtSolar(css)}</Text>
            </View>
          </View>
        </View>
...
function DayDetailSheet({ weather, solarTimes, currentPeriod, brahmaInfo, wakeLog, sunStreak, onMore, onClose, onShowShareCard }: {
  ...
  return (
    <>
    <Modal visible animationType="fade" transparent={true} statusBarTranslucent onRequestClose={close}>
      <Animated.View style={{ flex: 1, backgroundColor: '#0A0A0A' }}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
          ...
          <TouchableOpacity
            onPress={close}
            style={{ position: 'absolute', top: 20, right: 20, zIndex: 20, width: 36, height: 36, borderRadius: 18, borderWidth: 0.5, borderColor: 'rgba(212,175,55,0.4)', backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.4, shadowRadius: 8, overflow: 'hidden' }}>
            <BlurView intensity={30} tint="light" style={StyleSheet.absoluteFillObject} />
            <Text style={{ color: '#D4AF37', fontSize: 16, fontWeight: '400', opacity: 0.9 }}>✕</Text>
          </TouchableOpacity>

          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40, paddingTop: 0 }}>
            ...
            {/* WEATHER SECTION MOVED TO HEADER */}
...
function DailyTab() {
  ...
  const [showStressScanner, setShowStressScanner] = useState(false);

  const triggerInsightReveal = React.useCallback((isManual = false) => {
    if (insightTimeoutRef.current) clearTimeout(insightTimeoutRef.current);
    
    // If manual tap, show immediately, else wait 1s
  ...
  const [activeFestDetail, setActiveFestDetail] = useState<Festival | null>(null);
  const festivals = React.useMemo(() => getYearlyFestivals(new Date().getFullYear()), []);
  const todayFest = React.useMemo(() => {
    const today = new Date();
  ...
  const solarContext = solarTimes ? (() => {
    ...
    const ZENITH_WIN_BEFORE = 20 / 60;
    const ZENITH_WIN_AFTER = 2 / 60;
  ...
  const handleStressSoundPlay = React.useCallback((soundId: string) => {
    const sound = ALL_SLEEP_SOUNDS.find((s: any) => s.id === soundId);
    if (sound) {
      setShowStressScanner(false);
      setTimeout(() => playSound(sound as any, 20 * 60), 400);
    }
  }, [playSound]);
  ...
  return (
    <Animated.View style={[D.screen, { backgroundColor: accentColor, opacity: entranceAnim, transform: [{ scale: entranceAnim.interpolate({ inputRange: [0, 1], outputRange: [1.05, 1] }) }] }]}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={{ flex: 1 }}>
          {!solarTimes ? (
            ...
            <View style={{ flex: 1, justifyContent: 'space-between', alignItems: 'center', paddingBottom: insets.bottom + 52 }}>
                ...
                {todayFest && (
                  ...
                  >
                    <BlurView intensity={55} tint="dark" style={{
                      ...
                      borderRadius: 18, borderWidth: 0.5, borderColor: 'rgba(255,215,0,0.22)',
                      flexDirection: 'row', alignItems: 'center', gap: 8,
                    }}>
                      <Text style={{ fontSize: 14 }}>{todayFest.emoji}</Text>
                      <Text style={{ fontSize: 10.5, fontWeight: '700', color: 'rgba(255,220,100,0.90)', letterSpacing: 1.4 }}>
                        TODAY · {todayFest.name.toUpperCase().split(' / ')[0]}
                      </Text>
                    </BlurView>
                  </TouchableOpacity>
                )}
                  ...
                  <Animated.View 
                    ...
                    style={{ 
                      ...
                      transform: [
                        ...
                        { translateY: insightRevealAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -10] }) }
                      ]
                    }}
                  >
                    {weather && (
                      ...
                      >
                        <BlurView intensity={70} tint="dark" style={{
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                            ...
                            <Text style={{ fontSize: 10, fontWeight: '900', color: accentC, letterSpacing: 2.5, textTransform: 'uppercase' }}>Active Bio-State</Text>
                          </View>
                          {/* Two columns */}
                      ...
                      <BlurView intensity={55} tint="dark" style={{
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 18, paddingVertical: 13 }}>
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                            ...
                            <Text style={{ fontSize: 11, fontWeight: '800', color: 'rgba(255,255,255,0.90)', letterSpacing: 0.5 }}>Active Bio-State</Text>
                          </View>
                        </View>
                      </BlurView>
                    </TouchableOpacity>
      ...
      {activeFestDetail && (
        <FestivalDetailModal 
          festival={activeFestDetail} 
          onClose={() => setActiveFestDetail(null)} 
        />
      )}
      ...
      {/* ── Bio-Stress Scanner Modal ── */}
      <StressScanner
        visible={showStressScanner}
        onClose={() => setShowStressScanner(false)}
        onPlaySound={handleStressSoundPlay}
        accentColor={currentPeriod?.color ?? '#34d399'}
      />
    </Animated.View>
  );
}
...
const PC = StyleSheet.create({
  ...
  triEmoji:        { fontSize: 18, marginBottom: 2 },
  triTitle:        { fontSize: 11, fontWeight: '900' },
  triSub:          { fontSize: 9, color: '#8FB8D0AA', lineHeight: 14 },
  triEn:           { fontSize: 8, color: '#00D4B8AA', fontWeight: '700', marginTop: 3 },
  ...
  biSanskrit:      { fontSize: 12, fontWeight: '900' },
  biEnglish:       { fontSize: 11, color: '#FFFFFFEE', fontWeight: '800' },
  exploreBtn:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderRadius: 99, paddingHorizontal: 16, paddingVertical: 12, marginTop: 4, marginBottom: 2 },
  exploreTxt:      { fontSize: 10, fontWeight: '900', letterSpacing: 1.5, flex: 1, textTransform: 'uppercase' },
  exploreArrow:    { fontSize: 14, fontWeight: '900' },
});
...
// ── Crash-safe export: any render-time error in DailyTab shows a friendly
// "Try Again" overlay instead of crashing the whole app.
