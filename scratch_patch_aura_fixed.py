...
"""
    ...
    # Insert it into the render tree right after AppBackground
    render_target = """    <Animated.View style={[D.screen, { backgroundColor: accentColor, opacity: entranceAnim, transform: [{ scale: entranceAnim.interpolate({ inputRange: [0, 1], outputRange: [1.05, 1] }) }] }]}>
      <AppBackground />
      {/* Background filters removed per user request to show raw background image */}"""
      
    render_replacement = """    <Animated.View style={[D.screen, { backgroundColor: accentColor, opacity: entranceAnim, transform: [{ scale: entranceAnim.interpolate({ inputRange: [0, 1], outputRange: [1.05, 1] }) }] }]}>
      <AppBackground />
